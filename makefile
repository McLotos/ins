...
up: ## Запускаем сервисы
ifeq (dev, $(filter dev,$(MAKECMDGOALS)))
		@echo 'run services in dev mode'
		docker compose --env-file .env.dev --profile services up --build
else
		@echo 'run services in prod mode'
...
up-db: ## Запускаем базы данных
ifeq (dev, $(filter dev,$(MAKECMDGOALS)))
		@echo 'run databases in dev mode'
		docker compose --env-file .env.dev --profile db up --build
else
		@echo 'run databases in prod mode'
